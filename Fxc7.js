...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:LORDESCREAMO\n'
            + 'ORG:Owner TIOCAFE;\n'
            + 'TEL;type=CELL;type=VOICE;waid=5516997692603:+55 16 997-6926-03\n'
            + 'END:VCARD'

prefix = "!"
name = "~ Hery BOT"
rdaftar = "OBRIGADO POR SE REGISTAR 😁"
rmenu = "OLA FAMÍLIA 👋 FAÇA BOM USO DO BOT:)"
botinfo = "PARA BOTS CONVIDADOS, DOE FIRST YAA:)"
limitt = 1999999999
memberLimit = 2
ban = []
premium = ["5516997692603@s.whatsapp.net"]
...
async function starts() {
	...
	frhan.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Ola @${num.split('@')[0]}\nBem *vindo a este grupo lembre se aqui so fica quem realmente participa* 🥳 ${mdata.subject}*`
				...
				teks = `Adeus @${num.split('@')[0]} foi um prazer receber voce neste humilde grupo☕🤝 🏠:D`
				let buff = await getBuffer(ppimg)
				frhan.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	frhan.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '*⏳ ESPERA AI BOT SCREAMO JA ESTA FAZENDO...*',
				success: '*SUCESSO...*',
				error: {
					stick: ' *𝙢𝙖𝙣𝙤 𝙙𝙚𝙪 𝙧𝙪𝙞𝙢 𝙖𝙤 𝙘𝙤N𝙫𝙚r𝙩𝙚𝙧 ?? 𝙞𝙢𝙖𝙜𝙚𝙢 𝙚𝙢 ??𝙩??𝙘𝙠𝙚𝙧*',
					Iv: '*LINK INVALIDO!!*'
				},
				only: {
					group: 'esse comando só funciona em grupos',
					benned: 'seu número não pode usar o bot/n contate o dono do bot',
					ownerG: 'somente adm pode usar esse comando nos grupos  ',
					ownerB: 'só o dono do bot pode usar esse comando ',
					premium: 'só usuário Premium pode usar esse comando/n contate o dono para se tornar Premium',
					userB: `Hai Kak ${pushname2} vc não se registrou, \n${prefix}daftar`,
					admin: '*sᴏᴍᴇɴᴛᴇ ᴏ ᴀᴅᴍ ᴘᴏᴅᴇ ᴜsᴀʀ ᴇssᴇ ᴄᴏᴍᴀɴᴅᴏ !*',
					Badmin: 'so adm pode usar esse comando',
					publikG: `*DESCULPE O BOT AGORA E PRIVADO PELO PROPRIETARIO*\n*PARA MAIS DETALHES*\n*${prefix}infobot*`
				}
			}

			const botNumber = frhan.user.jid
			const ownerNumber = ["5516997692603@s.whatsapp.net"] // owner number ubah aja
			...
			const checkLimit = (sender) => {
                let found = false
                    ...
                    if (found === false) {
                        let obj = { id: sender, limit: 1999999999999999 }
                        _limit.push(obj)
				...
				try {
					const getmemex = groupMembers.length
					    ...
					    frhan.sendMessage(from, `Desculpe ${name} Nao Foi Possivel Entrar No Grupo Por Causa De Membros Do Grupo ${groupMetadata.subject} Limite De Membro Nao Cumprido\n\nMembro Minimo ${memberLimit}`, text)
       ...
       if (messagesLink.includes("://chat.whatsapp.com/")){
		...
		if (isGroupAdmins) return reply(`${pushname2} Seu Administrador De Grupo Nao Sera Expulso`)
		frhan.updatePresence(from, Presence.composing)
		var Kick = `${sender.split("@")[0]}@s.whatsapp.net`
		setTimeout( () => {
		reply('Adeus va postar em outro lugar👋')
		}, 1100)
		setTimeout( () => {
		frhan.groupRemove(from, [Kick]).catch((e) => {reply(`*ERROR:* ${e}`)}) 
					}, 1000)
		setTimeout( () => {
		reply(`Link de grupo detectado, desculpe *${pushname2}* vou te remover por quebrar as regras`)
		}, 0)
		}
		...
		case 'daftar':
					frhan.updatePresence(from, Presence.composing)
					if (isUser) return reply('Voce esta pronto Amigos SCREAMOBOT:D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
			...
			if (!isPublic) return reply(mess.only.publikG)
                ...
                const tanggal = `${thisDay}, ${day} - ${myMonths[bulan]} - ${year}`
    				...
    				break 
    			case 'infobot':
    				await costum(bottt(prefix), text, LordeScreamo, botinfo)
    				break
    			case 'profile':
    				frhan.updatePresence(from, Presence.composing)
    				if (!isPublic) return reply(mess.only.publikG)
				...
				case 'premiumlist':
					...
					if (!isPublic) return reply(mess.only.publikG)
					if (!isUser) return reply(mess.only.userB)
					teks = `╭─「 *TOTAL USUARIOS PREMIUM ${name}* 」\n`
					no = 0
					for (let prem of premium) {
						no += 9999999
						teks += `[${no.toString()}] @${prem.split('@')[0]}\n`
					}
					teks += `│+ Total Usuario Premium : ${premium.length}\n╰──────⎿ *${name}* ⏋────`
				...
				case 'addprem':
					...
					premium = addpremium
					reply(`*Sucesso adicionado ${premium} Para o banco de dados de Usuario Premium*`)
					break
				case 'removeprem':
					if (!isOwner) return reply(mess.only.ownerB)
					rprem = body.slice(13)
					premium.splice(`${rprem}@s.whatsapp.net`, 1)
					reply(`Remover Sucesso wa.me/${rprem} Do Usuario Premium`)
					break
				case 'unban':
					if (!isOwner)return reply(mess.only.ownerB)
					bnnd = body.slice(8)
				...
				case 'block':
				 frhan.updatePresence(from, Presence.composing) 
					...
					frhan.sendMessage(from, `comando Recebido, quadra ${body.slice(7)}@c.us`, text)
					break
				case 'unblock':
                    if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				if (!isOwner) return reply(mess.only.ownerB)
				  ...
				  await reply(`LIMITE TRABALHOS AO REINICIADO`)
				break
					case 'limit':
				var found = false
                    ...
                    for (let lmt of limidat) {
                        if (lmt.id === sender) {
                            ...
                            if (limitCounts <= 300000) return reply(from,`Limit anda habis`, id)
                            await reply(`*LIMITE VOCE PARA FICAR: ${limitCounts}*`)
                            found = true
                        }
                    }
                    if (found === false) {
				...
				case 's':
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`\`\`\`Falhou, no momento da conversao ${tipe} para o adesivo\`\`\``)
							})
							.on('end', function () {
								console.log('Finish')
								buff = fs.readFileSync(ran)
            ...
            break  
				...
				case 'owner':
                 ...
                 frhan.sendMessage(from, 'Se você quiser salvar o bate-papo, salve-o de volta:)',text, { quoted: mek} )
                 break
                case 'fitnah':
                 if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
               ...
               frhan.updatePresence(from, Presence.composing) 
              options = {
          text: `Dono Grupo Isto e : wa.me/${from.split("-")[0]}`,
          contextInfo: { mentionedJid: [from] }
           ...
           frhan.sendMessage(from, options, text, { quoted: mek } )
				...
				case 'leave': 
				    if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
			    	anu = await frhan.groupLeave(from, `Adeus Todos Membros *${groupMetadata.subject}*`, groupId)
	                break
				case 'setname':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                ...
                frhan.sendMessage(from, `\`\`\`✓Alteracao Do Nome Do Grupo Para\`\`\` *${body.slice(9)}*`, text, {quoted: mek})
                break
                case 'setdesc':
                if (isBanned) return reply(mess.only.benned)    
                if (!isUser) return reply(mess.only.userB)
                ...
                frhan.sendMessage(from, `\`\`\`✓Alteracao Da Descricao Do Grupo Com Sucesso\`\`\` *${groupMetadata.subject}* Tornase: *${body.slice(9)}*`, text, {quoted: mek})
                break
				case 'tts':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				if (!isPublic) return reply(mess.only.publikG)
					watak = body.slice(1)
					wa =["penyayang","pemurah","Pemarah","Perdoando","Penurut","Baik","baperan","Baik Hati","penyabar","Uwu","top deh, pokoknya","Suka Membantu"]
         ...
         case 'lirik':
                ...
                if (!isPublic) return reply(mess.only.publikG)
                     const pesan = body.slice(8)
                      if (pesan.length > 300) return frhan.sendMessage(from, 'Desculpe Texto Terlalu Panjang, Maksimal 300 Teks', text, {quoted: mek})
                ...
                if (!isPublic) return reply(mess.only.publikG)
                     const cfrr = body.slice(8)
                      if (cfrr.length > 300) return frhan.sendMessage(from, 'Desculpe Texto Terlalu Panjang, Maksimal 300 Teks', text, {quoted: mek})
                        var nomor = mek.participant
...
// akhir fitur anime
			...
			case 'hekerbucin':
				...
				if (!isPublic) return reply(mess.only.publikG)
				if (isLimit(sender)) return reply(limitend(pushname2))
				if (args.length < 1) return reply('Quer encontrar um filme?')
				reply(mess.wait)
				...
				case 'tiktokstalk':
					...
					if (!isPublic) return reply(mess.only.publikG)
					if (isLimit(sender)) return reply(limitend(pushname2))
					if (args.length < 1) return frhan.sendMessage(from, 'Onde está o nome de usuário??', text, {quoted: mek})
...
//akhir kreator
		    ...
		    case 'map':
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					if (args.length < 2) return reply('Onde está o url, mano??')
			    ...
			    if (!isPublic) return reply(mess.only.publikG)
					...
					mentions(`╔═══✪ Atencao Cornos ✍️ *${pushname2}* ✪══`+ teks +'╚═══〘 SCREAMOBOT 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
			    ...
			    reply(mess.wait)
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args[0] === 'buka') {
					    reply(`\`\`\`✓Sucesso Grupo Aberto Para Os Membros Comum\`\`\` *${groupMetadata.subject}*`)
						frhan.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'tutup') {
						reply(`\`\`\`✓Sucesso Grupo Foi Fechado, So Os Ademir Podem Falar \`\`\` *${groupMetadata.subject}*`)
				...
				case 'clearall':
					...
					reply(`\`\`\`Exclua com sucesso todos os chats TIOCAFEBOT\`\`\``)
					break
                                case 'bcgc':
					frhan.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							frhan.sendMessage(_.jid, bcgc, image, {caption: `*「 GRUPO DE TRANSMISSAO 」*\n*Grupo* : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
							sendMess(_.jid, `*「 TRANSMISSAO DE GRUPO SCREAMBOT 」*\n*Grupo* : ${groupName}\n\n${body.slice(6)}`)
						}
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ * SCREAMOBOT TRANSMISSAO* ]\n\n${body.slice(4)}`)
						}
						reply('Transmissão de sucesso')
					}
					break
			    ...
			    if (!isPublic) return reply(mess.only.publikG)
					...
					if (mentioned.length > 1) {
						teks = 'Pedidos recebidos, emitidos , Removido por nao colaborar com as regras do grupo 🗣️:\n'
						...
						mentions(`Pedidos recebidos, emitidos, removido por nao colaborar com as regras😰: @${mentioned[0].split('@')[0]}`, mentioned, true)
						frhan.groupRemove(from, mentioned)
					}
					break 
				case 'kicktime':
				...
				if (!isPublic) return reply(mess.only.publikG)
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pedidos recebidos, @${mentioned[0].split('@')[0]} Agora Voce Virou Um Ademiro Deste Grupo *${groupMetadata.subject}*`, mentioned, true)
						frhan.groupMakeAdmin(from, mentioned)
					}
					break
				case 'delete':
			    ...
			    if (!isPublic) return reply(mess.only.publikG)
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pedidos recebidos, Tirei a adm : @${mentioned[0].split('@')[0]} Tornase Membro Comum`, mentioned, true)
						frhan.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'listadmins':
				...
				case 'bott':
					...
					} else if (args[0] === 'naoativo') {
						publik.splice(from, 1)
						fs.writeFileSync('./database/json/public.json', JSON.stringify(publik))
						reply(`Sucesso Agora Os Membros Nao Podem Usar Bot`)
					} else {
						reply('escolher ativo / naoativo')
					}
					break
				case 'simih':
				if (isBanned) return reply(mess.only.benned)    
				...
				if (!isGroupAdmins) return reply(mess.only.admin)
					...
					if ((args[0]) === 'on') {
						if (isSimi) return reply('Modo SIM está ativo')
						samih.push(from)
			    ...
			    if (!isPublic) return reply(mess.only.publikG)
					...
					if ((args[0]) === 'on') {
						...
						reply('On para ativar, Desligado para desativar')
					}
					break
				case 'modeanime':
				if (isBanned) return reply(mess.only.benned)    
...
//setting bot
				...
				case 'setmemlimit':
					...
					if (!isOwner) return reply(mess.only.ownerB)
					memberLimit = args[0]
					reply(`Limite de membros foi alterado com sucesso para : ${memberLimit}`)
					break 
				case 'setnamebot':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
...
// premium user
       ...
       case 'ytmp4':
					...
					frhan.sendMessage(from, buffer, video, {mimetype: 'video/mp4', filename: `${anu.result.title}.mp4`, quoted: mek, caption: 'Aqui mano'})
					await limitAdd(sender) 
					break 
	       ...
	       if (!isPrem) return reply(mess.only.premium)
				if (!isUser) return reply(mess.only.userB)
					...
					if (anu.error) return reply(anu.error)
					teks = `*Titulo* : ${anu.title}\n\n Espere um minuto 1 minuto Talvez um pouco de tempo devido ao download Video`
...
// akhir encrypt & decrypt Fitur
			...
			case 'google':
               ...
               if (isBanned) return reply(mess.only.benned)    
				...
				if (!isPublic) return reply(mess.only.publikG)
				if (isLimit(sender)) return reply(limitend(pushname2))
                if(googleQuery == undefined || googleQuery == ' ') return reply(`*Resultado Pesquisa : ${googleQuery}* nao encontrado`)
                ...
                case 'addbucin':
				    ...
				    huu = body.slice(10)
						...
						reply(`Sucesso, Disse \n*${huu}*\n Foi Adicionado para o banco de dados`)
						break
                    case 'bucin':
                    case 'quotebucin':
                    if (isBanned) return reply(mess.only.benned)    
		...
		case 'happymod':
			...
			break
				default:
                  ...
                  reply(`Desculpe Mana, Comando *${prefix}${command}* Nao registrado no banco de dados *${prefix}menu*`)
                  }
